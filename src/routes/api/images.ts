...
const validate = async (query: ImageQuery): Promise<null | string> => {
  ...
  if (!(await control.isImageAvailable(query.filename))) {
    const availableImageNames: string = (
      await control.getAvailableImageNames()
    ).join(", ");
...
images.get(
  ...
  async (
    ...
    if (!(await control.isThumbAvailable(request.query))) {
      error = await control.createThumb(request.query);
    }
    ...
    const path: null | string = await control.getImagePath(request.query);
    if (path) {
